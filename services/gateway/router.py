...
import time
import asyncio
from .config import GOVERNANCE_BASE, OPTIMIZER_BASE, LLM_BASE, KPI_BASE, POLICY_BASE, TWIN_BASE, PILOT_BASE, EVIDENCE_BASE, OT_BASE, GATEWAY_SYSTEM_TOKEN
...
async def status():
    statuses = {}
    for name, base in [("governance", GOVERNANCE_BASE), ("optimizer", OPTIMIZER_BASE), ("llm", LLM_BASE), ("kpi", KPI_BASE), ("policy", POLICY_BASE), ("twin", TWIN_BASE), ("evidence", EVIDENCE_BASE), ("ot", OT_BASE)]:
        try:
...
async def proxy(path: str, request: Request):
    ...
    if full_path.startswith("/corridor/") or full_path.startswith("/mode/") or full_path.startswith("/governance/"):
        ...
        raise HTTPException(status_code=404, detail=f"No microservice registered for path: {full_path}")
